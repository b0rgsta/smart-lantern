...
TrailsEffect::TrailsEffect(LEDController &ledController, int maxTrails, int trailLength) : Effect(ledController),
    maxTrails(maxTrails),
...
void TrailsEffect::update() {
    ...
    if (random(100) == 0) {
        // Reduced from 10% to 1% since we're updating 3x more often
        createNewTrail();
    }
    ...
    for (int i = 0; i < maxTrails; i++) {
        if (trails[i].active) {
            ...
            if (random(600) == 0) {
                // Changed from 50 to 600 to maintain same deactivation rate
                trails[i].active = false;
            }
        }
    }
