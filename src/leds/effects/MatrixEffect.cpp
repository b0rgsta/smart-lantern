...
void MatrixEffect::updateColorPalette() {
    ...
    for (int i = 0; i < NUM_COLORS; i++) {
        ...
        // Set to HSV color
        colorPalette[i] = leds.colorHSV(hue, 200, 255); // Slightly desaturated
    }
}
...
void MatrixEffect::renderDrop(Drop& drop, int stripType, int subStrip, int stripLength) {
    ...
    // Select the appropriate LED strip
    switch (stripType) {
        case 0:
            strip = &leds.getCore();
            break;
    ...
    if (headPos >= 0 && headPos < stripLength) {
        ...
        if (drop.isWhite) {
            ...
            strip->setPixelColor(physicalPos, strip->Color(whiteBright, whiteBright, whiteBright));
        } else {
            // Colored drop
            uint32_t color = colorPalette[drop.hue];
            
            // Apply brightness adjustment
            uint8_t r = ((color >> 16) & 0xFF) * drop.brightness / 255;
            uint8_t g = ((color >> 8) & 0xFF) * drop.brightness / 255;
            uint8_t b = (color & 0xFF) * drop.brightness / 255;
            
            strip->setPixelColor(physicalPos, strip->Color(r, g, b));
        }
    }
    ...
    for (uint8_t i = 1; i <= TRAIL_LENGTH; i++) {
        ...
        if (trailPos >= 0 && trailPos < stripLength) {
            ...
            strip->setPixelColor(physicalPos, strip->Color(trailBright, trailBright, trailBright));
        }
    }
}